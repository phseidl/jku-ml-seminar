...
class SincConv_fast(nn.Module):
    ...
    def __init__(self, out_channels, kernel_size, sample_rate=200, in_channels=1,
                 stride=1, padding=0, normalize=None, slice_len=280, dilation=1, bias=False, groups=1, min_low_hz=0,
                 min_band_hz=2):
...
class sincnet_conv_layers(nn.Module):
    def __init__(self, args: dict):
        ...
        for i in range(self.cnn_layer_num):
            ...
            current_input = int((current_input - cnn_kernel_sizes[i] + 1) / self.stride_list[i])
...
class SINCNET_FEATURE(nn.Module):
    def __init__(self, args: dict, num_eeg_channel):
