    ...
    def __init__(self, inp, hidden_dim, oup, kernel_size, stride, use_se, use_hs, is_psd):
        ...
        if inp == hidden_dim:
            self.conv = nn.Sequential(
                nn.Conv2d(hidden_dim, hidden_dim, (1, kernel_size ** 2), (1, stride), (0, (kernel_size - 1) // 2),
            ...
            self.conv = nn.Sequential(
                ...
                h_swish() if use_hs else nn.ReLU(inplace=True),
                # dw
                nn.Conv2d(hidden_dim, hidden_dim, (1, kernel_size), (1, stride), (0, (kernel_size - 1) // 2),
...
class MobileNetV3(nn.Module):
    def __init__(self, args: dict, device):
        ...
        output_channel = _make_divisible(output_channel[mode] * self.width_mult, 8) if self.width_mult > 1.0 else \
        output_channel[mode]
