...
from UPT4EEG_ensemble_loss.utils.bin_loss import binned_cross_entropy_loss
...
class Trainer:
    def __init__(self, model, optimizer, loss_fn, device="cpu", use_wandb=False, wandb_project_name="UPT4EEG"):
        ...
        bin_size = 128
        #self.bin_edges = torch.arange(-1.2, 1.2 + bin_size, bin_size) 
        self.bin_edges = torch.linspace(-1.2, 1.2, steps=bin_size) 
    ...
    def train(self, train_dataloader, val_dataloader, num_epochs, train_config):
        ...
        for epoch in range(num_epochs):
            ...
            for i, batch in enumerate(train_dataloader):
                ...
                loss_bin = 0.08 * binned_cross_entropy_loss(y_hat, y, self.bin_edges.to(self.device))
                loss_mse = self.loss_fn(y_hat, y)
                loss = loss_bin + loss_mse
                ...
                if self.use_wandb:
                    run.log({
                        "update": update,
                        "training_loss_ensemble": loss.item(),
