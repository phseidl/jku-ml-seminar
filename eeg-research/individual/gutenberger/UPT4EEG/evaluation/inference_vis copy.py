...
vis_dataset = SparseEEGDataset_val(x_vis, y_vis, query_vis, ch_names, cfg_dataset, use_montage=use_montage, io_same=io_same)
...
viewARA(
    ...
    [x_data, y_data, y_data, p_data], 1, electrode, MODEL_CLASS,
    titles=["Noisy", "", "Clean", MODEL_CLASS], colors=["gray", "gray", "red", "blue"], alphas=[0.5, 0, 0.8, 0.8], ax=ax,
    picks_chs = picks_chs
)
plt.savefig("UPT4EEG/evaluation/plots/inference_rdmMont_S" + subject_vis[0] + "_" + model_weights_name+".pdf", format="pdf", bbox_inches="tight")
plt.show()
