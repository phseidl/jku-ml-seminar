...
query1 = {'query_freq': 50.0, 'query_montage_pairs': query_montage_pairs}
...
for query in query_list:
    ...
    for (channel_pos, data_pred), (channel_pos_tar, data_tar), (channel_pos_x, data_x) in zip(channel_data.items(), channel_data_tar.items(), channel_data_x.items()):
        idx += 1
        if idx == 11:
            ...
            plt.plot(t_tar/173, data_tar, label=f'Target (' + str(int(SFREQ)) + ' Hz)', color='red', linewidth=3, alpha = 0.8)
            ...
            plt.savefig("UPT4EEG/evaluation/plots/freq_independence_plot_S" + subject_vis[0] + "_" + str(query["query_freq"]) + "_" + str(idx) + ".pdf", format="pdf", bbox_inches="tight")
        plt.show()