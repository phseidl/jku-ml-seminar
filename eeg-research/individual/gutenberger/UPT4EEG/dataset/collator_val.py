...
class SparseEEG_Collator:
    ...
    def __call__(self, batch):
        ...
        # output_pos
        collated_batch["target_pos"] = default_collate([batch[i]["target_pos"] for i in range(len(batch))])

        # query pos
        collated_batch["query_pos"] = default_collate([batch[i]["query_pos"] for i in range(len(batch))])

        # normalization factor (percentiles)
        collated_batch["norm_factor"] = default_collate([batch[i]["norm_factor"] for i in range(len(batch))])
