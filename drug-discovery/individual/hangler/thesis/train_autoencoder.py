...
class ldmol_autoencoder(pl.LightningModule):
    def __init__(self, cp=None, config=None, loader_len=0, no_train=False, tokenizer=None, use_linear=True, use_precomputed=True):
        super().__init__()
    ...
    def forward(self, text_input_ids, text_attention_mask, embeddings=None, text_input_ids2=None, text_attention_mask2=None):
        # ================= MLM ================= #
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        if self.use_precomputed:
            text, embeddings = train_batch
            text_input = self.tokenizer(text, padding='longest', truncation=True, max_length=128, return_tensors="pt").to(self.device)
            loss_mlm = self(text_input.input_ids[:, 1:], text_input.attention_mask[:, 1:], embeddings=embeddings.to(self.device))
        else:
            text = train_batch
            text_input = self.tokenizer(text, padding='longest', truncation=True, max_length=128, return_tensors="pt").to(self.device)
